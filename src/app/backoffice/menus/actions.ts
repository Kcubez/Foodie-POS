...
const FormSchema = z.object({
  id: z.number({ message: 'Required field id is missing' }).gt(0, { message: 'Id cannot be 0' }),
  ...
  imageUrl: z.string(),
  menuCategoryIds: z.array(z.number()).min(1, { message: 'Required menu category ids is missing' }),
});
...
export async function updateMenu(formData: FormData) {
  try {
    ...
    const addonCategoryIds = formData.getAll('addonCategories').map(item => Number(item));
    ...
    const isSameMenuCategoryIds =
      ...
      menuCategoryIds.every((itemId: number) => existingMenuCategoryIds.includes(itemId));
    ...
    const existingAddonCategoryIds = menusAddonCategories.map(item => item.addonCategoryId);
    const isSameAddonCategoryIds =
      addonCategoryIds.length === existingAddonCategoryIds.length &&
      addonCategoryIds.every((itemId: number) => existingAddonCategoryIds.includes(itemId));
    ...
    if (!isAvailable) {
      ...
      const disabledLocationsMenus = await prisma.disabledLocationsMenus.findFirst({
        where: { menuId: Number(id) },
      });
    ...
    return {
      errors: [{ message: 'Something went wrong. Please contact our support.' }],
    };
  }
...
export async function createMenu(formData: FormData) {
  try {
    const { name, price, menuCategoryIds, imageUrl } = CreateMenuValidate.parse({
      name: formData.get('name'),
      menuCategoryIds: formData.getAll('menuCategories').map(item => Number(item)),
      price: Number(formData.get('price')),
      imageUrl:

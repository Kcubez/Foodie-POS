...
export default async function MenuUpdatePage({ params }: Props) {
  ...
  const isAvailable = menu.DisabledLocationsMenus.find(item => item.menuId === Number(id))
    ? false
    : true;
  const menuCategories = await getCompanyMenuCategories();
  const selectedMenuCategoryIds = menu?.menuCategoriesMenus.map(item => item.menuCategoryId);
  const addonCategories = await getCompanyAddonCategories();
  const selectedAddonCategoryIds = menu?.menusAddonCategories.map(item => item.addonCategoryId);

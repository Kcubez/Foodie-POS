...
export default async function OrdersWithStatusPage({ params }: Props) {
  ...
  const orders: OrdersWithMenusTablesOrderAddons[] = await prisma.orders.findMany({
    where: {
      tableId: { in: tableIds },
    ...
    <Box>
      <ButtonGroup variant="outlined" sx={{ display: 'flex', justifyContent: 'flex-end' }}>
        <Link href={'/backoffice/orders/pending'}>
          <Button variant={`${status === ORDERSTATUS.PENDING ? 'contained' : 'outlined'}`}>
            PENDING
          </Button>
        </Link>
        <Link href={'/backoffice/orders/cooking'}>
          <Button variant={`${status === ORDERSTATUS.COOKING ? 'contained' : 'outlined'}`}>
            COOKING
          </Button>
        </Link>
        <Link href={'/backoffice/orders/complete'}>
          <Button variant={`${status === ORDERSTATUS.COMPLETE ? 'contained' : 'outlined'}`}>
            COMPLETE
          </Button>
        </Link>
      </ButtonGroup>
      <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
        {orders.map(async order => {
          ...
          return <OrderCard key={order.id} order={order} addons={addons} isAdmin />;
        })}
      </Box>
    </Box>
  );

      ...
      {addonCategoryAddons.map(addonCategoryAddon => {
        return (
          ...
          >
              ...
              control={
                addonCategory.isRequired ? (
                  <Radio
                    ...
                    checked={
                      selectedAddons.find(item => item.id === addonCategoryAddon.id) ? true : false
                    }
                    onChange={() => {
                      const addonIds = addonCategoryAddons.map(item => item.id);
                      const others = selectedAddons.filter(item => !addonIds.includes(item.id));
                  ...
                  <Checkbox
                    ...
                    onChange={(evt, value) => {
                      if (value) {
                        setSelectedAddons([...selectedAddons, addonCategoryAddon]);
                      } else {
            ...
            />
            <Typography sx={{ fontStyle: 'italic' }}>{addonCategoryAddon.price}</Typography>
          </Box>
        );
      })}
    </Box>

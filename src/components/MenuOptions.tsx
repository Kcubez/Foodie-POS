...
import { AddonCategories as AddonCategoriesType, Addons, Menus, Orders } from '@prisma/client';
...
export function MenuOptions({ menu, addonCategories, addons, tableId, order }: Props) {
  const [quantity, setQuantity] = useState(1);
  const [selectedAddons, setSelectedAddons] = useState<Addons[]>([]);
  const [isDisabled, setIsDisabled] = useState(true);

  useEffect(() => {
    const requiredAddonCategories = addonCategories.filter(item => item.isRequired);
    ...
    const isDisabled = requiredAddonCategories.length !== selectedRequiredAddons.length;
